...
class GetVenda(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(self, text="Id da Venda:").grid(row=1, column=0, padx=10, pady=5, columnspan=4)
        tk.Entry(self, textvariable=self.idVenda, width=30 ).grid(row=2, column=0, padx=10, pady=15, columnspan=4)
        tk.Button(self, text="Buscar", command=self.buscar_venda, width=25 ).grid(row=3, column=0, columnspan=4, pady=3)
    ...
    def buscar_venda(self):
        try:
            ...
            if venda:
                tk.messagebox.showinfo("Sucesso", f"Venda encontrada!\n\n Id da Venda:{venda.idVenda}\n Vendedor: {venda.cpfVendedor}\n Comprador:{venda.cpfCompra}\n Chassi:{venda.numeroChassi}\n Data:{venda.data}\n Preço:{venda.preco}")
                self.idVenda.set("")
            else:
